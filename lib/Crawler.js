...
module.exports = class Crawler {
            ...
            return await this.withPage(browser)(async (page) => {
                ...
                const lessons = await Promise
                    .mapSeries(courses, async course => {
                        ...
                        logger.log('lessons length:', lessons, lessons.length, 'concurrency:', concurrency)
                        // lessons = lessons.slice(0, concurrency || Infinity).concat(lessons.slice(concurrency || Infinity).reverse())
                        return await Promise
                            .map(lessons, async (lesson, index) => {//.slice(0, 10)
                                return await this.withPage(browser)(async (page) => {
                                    ...
                                    const [vimeoUrl, data] = await Promise.all([
                                        (async () => {
                                            try {
                                                ...
                                                // logger.log('lessonIcon:::::', lessonIcon)
                                                if (lessonIcon !== '#icon__Video') {
                                                    logger.warn('not video lesson:', lessonIcon, lesson.url)
                                                    return
                                                }
                                                ...
                                                return await this.retry(async () => {
                                                    ...
                                                    logger.log(`[scraping:] ${lesson.position} => ${lesson.title} - ${lesson.url} - ${vimeoUrl}`)
                                                    return vimeoUrl
                                                }, 6, 1e3, true, page)
                                    ...
                                    resource?.vimeoUrl && await this.d(filename, prefix, [resource], { ms, ...opts })
    ...
    async d (filename, prefix, courses, opts) {
        ...
        await Promise.map(courses, async (course) => {
            ...
            if (!course?.downPath) {
                logger.log('dest:', opts.dir, course.downPath)
                logger.log('cccccc', course)
            }
            ...
            if (file) {
                courses[course.position].done = true
        ...
        try {
            ...
            if (retriesLeft) {
                ...
                if (page) {
                    ...
                    await page?.screenshot({
                        path    : path.resolve(process.cwd(), `errors/${new Date().toISOString()} - ${browserPage}.png`),
                        fullPage: true
                    })
                }
