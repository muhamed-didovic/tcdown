...
module.exports = class Crawler {
            ...
            return await this.withPage(browser)(async (page) => {
                ...
                const lessons = await Promise
                    .mapSeries(courses, async course => {
                        ...
                        lessons = FileChecker.getDownloadedFilenames(lessons, course.title, opts)
                                        ...
                                        (async () => {
                                            try {
                                                ...
                                                if (lessonIcon !== '#icon__Video') {
                                                    logger.warn('not video lesson:', lessonIcon, lesson.url, lesson.title)
                                                    return
                                                }
    ...
    async d (filename, prefix, courses, opts) {
        ...
        await Promise.map(courses, async (course) => {
            logger.info(`[d] Starting download with concurrency: ${concurrency} course: ${course.title}...`)
            if (course.done) {
                logger.log('[d] DONE for:', course.title)
                cnt++
                return
            }
            /*if (!course.vimeoUrl) {
            ...
            if (!course?.downPath) {
                logger.log('[d] dest:', opts.dir, course.downPath)
                logger.log('[d] details:', course)
            }
