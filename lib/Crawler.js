...
module.exports = class Crawler {
    ...
    async loginAndRedirect (page, opts) {
        ...
        await this.screenshotDebug(page, '11-')
        ...
        await this.screenshotDebug(page, '12-')
        await page.waitForSelector('input[type="email"]', { timeout: 200e3 })
            ...
            return await this.withPage(browser)(async (page) => {
                ...
                const lessons = await Promise
                    .mapSeries(courses, async course => {
                        ...
                        await page.goto(course.url, { waitUntil: 'networkidle0', timeout: 31e3 }) // wait until page load
    ...
    async screenshotDebug (page, title = '') {
        await fs.ensureDir(path.resolve(process.cwd(), 'debug'))
        await page.screenshot({
            path    : path.resolve(process.cwd(), `debug/${title}${new Date().toISOString()}.png`),
            fullPage: true
        })
    }
