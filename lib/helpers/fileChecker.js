...
module.exports = class FileChecker {
    ...
    static getDownloadedFilenames(lessons, title, opts) {
        // const dest = path.join(opts.dir, title)
        const logFile = path.join(opts.dir, title, 'videos.txt')
        // console.log('aaaa', logFile, fs.existsSync(logFile));
        if (!fs.existsSync(logFile)) return lessons;
        ...
        const sanitizeFilenames = fileNames.map(fileName => ({'title': fileName.split('/').pop().replace('.mp4', '').replace(/^\d+-/, '')}));
        ...
        //     position: 94
        // },
        const links = differenceBy(lessons, sanitizeFilenames, 'title')
